...
export function getRootPathOfMessage(message: LinterMessage): string {
  return atom.project.relativizePath($file(message) || '')[0]
}
...
export function filterMessagesByProject(
  messages: Array<LinterMessage>,
  filePath: ?string,
): Array<LinterMessage> {
  const filtered = []
  messages.forEach(function(message) {
    if (filePath === null || filePath.startsWith(getRootPathOfMessage(message))) {
